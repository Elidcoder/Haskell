...
update addedState@(id, _) otherStates
  = addedState : filter ((/= id) . fst) otherStates
--Add | Mul | Eq | Gtr 
apply :: Op -> Int -> Int -> Int
apply Add a b = a + b
apply Mul a b = a * b
apply Eq  a b = fromEnum (a == b)
apply Gtr a b = fromEnum (a > b)

eval :: Exp -> State -> Int
-- Pre: the variables in the expression will all be bound in the given state 
-- Pre: expressions do not contain phi instructions
eval (Const val) _ = val
eval (Var id) states = lookUp id states
eval (Apply op exp1 exp2) states = apply op (eval exp1 states) (eval exp2 states)
--no phi functions needed in pattern match

execStatement :: Statement -> State -> State
execStatement (Assign id exp) states = update (id, eval exp states) states
execStatement (If exp trueBlock falseBlock) states 
  | toEnum (eval exp states) = execBlock trueBlock states
  | otherwise = execBlock falseBlock states
execStatement doWhile@(DoWhile block exp) states 
  | toEnum (eval exp newStates) = execStatement doWhile newStates
  | otherwise = newStates
    where
      evalBlock = execBlock block states
